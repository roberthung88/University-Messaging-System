...
// ClassroomPage provides an info/settings page for the class.
// In the current implementation, ClassroomPage receives
...
// ChatPage to ClassroomPage via props. However, if we ever decide
// to implement ClassroomPage as its own page, we'll most likely
      ...
      <div className={classes.container}>
        ...
        {this.state.editing ? 
          <div style={{display: 'block'}}>
            <FormControlLabel style={{marginLeft: 'auto', marginRight: 'auto'}} control={<Checkbox checked={this.state.privateCheckbox} onChange={this.handleCheckedChange('privateCheckbox')} name="privateCheckbox" color="primary"/>} label={<Typography variant="h6" className={classes.basicLabel} style={{paddingRight: 9}}>Private Class</Typography>} />
          </div>
          :
          <Typography variant="h6" className={classes.basicLabel}>{this.state.chatRoom.private ? 'Private Class' : 'Public Class'}</Typography>
        }
        ...
        <MemberView type="Class" chatroomId={this.state.chatroomId} userIsAdmin={this.state.userIsAdmin} />
        <div>
          <IconButton className={classes.button} onClick={this.props.hideClassroom} color="primary" component="span" disabled={this.state.editing}>
            <ChatIcon />
          </IconButton>
          <IconButton className={classes.button} onClick={() => {this.props.hideClassroom(); this.props.showQueue();}} color="primary" component="span" disabled={this.state.editing} >
            <QueueIcon />
          </IconButton>
        </div>
      </div>
...
export default withStyles(useStyles)(ClassroomPage);
